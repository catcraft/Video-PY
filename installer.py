...
def installico(path):
        ...
        file_name = os.path.basename(url)
        open(path + "\\" + file_name, 'wb').write(r.content)
def installhtml(path):
        url = 'https://raw.githubusercontent.com/catcraft/Video-PY/main/Install/files/Templates/index.html'
        r = requests.get(url, allow_redirects=True)
        file_name = os.path.basename(url)
        open(path + "\\" + file_name, 'wb').write(r.content)
...
def download():
    try:
        ...
        #Install the Config file from Github
        user_doc = os.path.join(os.path.expanduser("~"), 'Documents', 'py')
        templates_dir = os.path.join(user_doc, 'Templates')
        os.makedirs(templates_dir, exist_ok=True)
        py_dir = os.path.join(user_doc, 'index.html')
        if os.path.isfile(py_dir) == True:
            os.remove(py_dir)
            installhtml(templates_dir)
        else:
           installhtml(templates_dir)
...
checkbox = tk.Checkbutton(root, text="Create Shortcut on Desktop", variable=checkvar, bg="#ffffff")
checkbox.place(rely=0.65, relx=0.3)
